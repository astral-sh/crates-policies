...
class Plan:
    ...
    def up_to_date(self) -> bool:
        return not (
            self.configs_to_delete or self.add_publisher or self.set_trustpub_only
        )
...
def get_crate_metadata(client: httpx.Client, crate: str) -> dict[str, Any]:
    response = client.get(f"{CRATES_IO_API}/crates/{crate}")
    if response.status_code == 404:
        raise ValueError(
            f"{crate}: crate is not published; trusted publishing requires an initial publish"
        )
...
def plan_crate(
    ...
    return Plan(
        ...
        set_trustpub_only=bool(metadata.get("trustpub_only")) != policy.trustpub_only,
    )
...
def main() -> int:
    ...
    try:
        ...
        if not token:
            raise ValueError(
                "CARGO_REGISTRY_TOKEN is required with the `trusted-publishing` scope"
            )
        ...
        with (
            ...
            for policy in policies:
                for crate in policy.crates:
                    ...
                    configs = list_trusted_publishers(auth_client, crate)
        ...
        if exc.response.status_code in {401, 403}:
            print(
                "hint: check that the token has the `trusted-publishing` scope and "
